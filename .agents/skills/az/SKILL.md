...
# Principle of least privilege: use the narrowest role and scope needed.
# Avoid Contributor at subscription scope; prefer resource-group or resource scope.
...
# NOTE: Connection strings contain full account access keys. Prefer managed identity
# or SAS tokens for applications. Use connection strings only for trusted admin tooling.
az storage account show-connection-string --name <account-name>

# Get access keys
# NOTE: Access keys grant full control over the storage account. Rotate regularly,
# and prefer managed identity or SAS tokens for application access.
...
# Create NSG rule
# WARNING: '*' for source-address-prefixes allows traffic from ANY IP address.
# In production, replace '*' with specific IP ranges (e.g. '10.0.0.0/8' or '<your-ip>/32').
...
# Create AKS cluster
# Add --api-server-authorized-ip-ranges to restrict access to the Kubernetes API server
az aks create \
  --resource-group <group> \
  --name <cluster-name> \
  --node-count 3 \
...
# Create key vault
# Use --enable-purge-protection for production vaults to prevent permanent deletion
az keyvault create \
  --name <vault-name> \
  --resource-group <group> \
  --location eastus \
...
# Set secret
# WARNING: --value exposes the secret in shell history. Prefer --file or an env var:
#   echo -n "$MY_SECRET" > /tmp/secret.txt && az keyvault secret set ... --file /tmp/secret.txt && rm /tmp/secret.txt
# Or prefix the command with a space (requires HISTCONTROL=ignorespace) to exclude it from history.
az keyvault secret set --vault-name <vault-name> --name <secret-name> --value "$MY_SECRET"
...
az network nsg rule create --resource-group vm-rg --nsg-name myNSG --name AllowSSH --priority 1000 --destination-port-ranges 22 --protocol Tcp --access Allow --source-address-prefixes <your-ip>/32  # restrict to your IP, never use *
...
# 4. Add secret to Key Vault (never hardcode secret values — use an env var or --file)
az keyvault secret set --vault-name myfuncvault --name DatabasePassword --value "$DB_PASSWORD"
...
3. **Enable purge protection on Key Vaults** for production
   ```bash
   az keyvault create --name <vault> --resource-group <rg> --location eastus --enable-purge-protection true
   ```
4. **Use Azure RBAC** for fine-grained access control; apply least-privilege (narrow role + scope)
5. **Regularly rotate credentials** for service principals
6. **Enforce HTTPS and minimum TLS 1.2** on all storage accounts
   ```bash
   az storage account update --name <account> --resource-group <rg> --https-only true --min-tls-version TLS1_2
   ```
7. **Never pass secrets as plain `--value` on the command line** — use `--file` or an env var to avoid shell history leakage:
   ```bash
   az keyvault secret set --vault-name <vault> --name <secret> --value "$MY_SECRET"
   # or: --file /path/to/secret.txt
   ```
8. **Restrict NSG rules and AKS API server access** to specific IP ranges instead of `*`
